...
static func find_path(
	...
	flying: bool,
	max_jump_height: int,
	max_search_radius: int = 40 # Hard limit for safety
) -> PackedVector2Array:
	
	var dist_x = abs(start_tile.x - goal_tile.x)
	var dist_y = abs(start_tile.y - goal_tile.y)
	
	if dist_x > max_search_radius or dist_y > max_search_radius:
		return PackedVector2Array() # Bail before allocating anything
